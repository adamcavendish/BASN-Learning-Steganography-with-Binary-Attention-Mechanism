...
def metrics_update(metrics, n=1):
  """Update metrics with a timed context manager (n is the batch size)"""

  class _MetricsContext(object):
    ...
    def __exit__(self, exc_type, exc_value, traceback):
      metrics.update(time.time() - self.split, self.n)

  return _MetricsContext(metrics, n)
